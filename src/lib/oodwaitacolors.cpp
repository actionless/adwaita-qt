...
#include "oodwaitacolors.h"
#include "oodwaitacolors_p.h"
#include "oodwaitadebug.h"
#include "animations/oodwaitaanimationdata.h"
...
Q_LOGGING_CATEGORY(OODWAITA, "oodwaita.colors")
...
static ColorsPrivate::OodwaitaColor colorNameToEnum(const QString &name)
{
    Q_ASSERT(!name.isEmpty());

    QMetaObject metaObject = ColorsPrivate::staticMetaObject;

    QMetaEnum metaEnum = metaObject.enumerator(metaObject.indexOfEnumerator("OodwaitaColor"));
    ColorsPrivate::OodwaitaColor value = static_cast<ColorsPrivate::OodwaitaColor>(metaEnum.keyToValue(name.toLatin1()));
...
ColorsPrivate::ColorsPrivate()
{
    ...
    for (const QString &variant : variants) {
        ...
        if (variant == QStringLiteral("light")) {
            colorVariant = Oodwaita;
        } else if (variant == QStringLiteral("dark")) {
            colorVariant = OodwaitaDark;
        } else if (variant == QStringLiteral("hc")) {
            colorVariant = OodwaitaHighcontrast;
        } else {
            colorVariant = OodwaitaHighcontrastInverse;
        }
		...
		if (theme_name.size()) {
			...
			if (!file.open(QIODevice::ReadOnly)) {
				...
				filename = QStringLiteral(":/stylesheet/Oodwaita-%1.css").arg(variant);
				qDebug() << QString("%1 Loading %2...").arg(log_prefix, filename);
				file.setFileName(filename);
				if (!file.open(QIODevice::ReadOnly)) {
					qWarning() << QString("%1 Can't open %2").arg(log_prefix, filename);
        ...
        while (!in.atEnd()) {
            ...
            if (line.startsWith(QStringLiteral("@define-color"))) {
                ...
                const OodwaitaColor colorEnum = colorNameToEnum(reMatch.captured(1));
                        ...
                        if (propertyName == QStringLiteral("background-image") ||
                            ...
                            if (propertyValue.startsWith(QStringLiteral("linear-gradient"))) {
                                ...
                                if (reMatch.hasMatch()) {
                                    ...
                                    // qCDebug(OODWAITA) << "Unhandled property value" << propertyValue << " for " << propertyName << " in " << fullButtonColorName;
                                 }
                            } else {
                            ...
                            // qCDebug(OODWAITA) << "Unhandled property " << propertyName << " in " << line;
                        }
                    }
                }
            }
...
QColor ColorsPrivate::oodwaitaColor(OodwaitaColor color, ColorVariant variant)
{
    return m_colors.value(color).value(variant);
}

QColor ColorsPrivate::oodwaitaWidgetColor(const QString &color, ColorVariant variant)
{
    ...
    if (!returnValue.isValid()) {
        ...
        return oodwaitaWidgetColor(baseColorName, variant);
    }
...
QPalette Colors::palette(ColorVariant variant)
{
    if (variant == ColorVariant::Unknown) {
        variant = ColorsPrivate::isDarkMode() ? ColorVariant::OodwaitaDark : ColorVariant::Oodwaita;
    }
    ...
    palette.setColor(QPalette::All,      QPalette::WindowText,      colorsGlobal->oodwaitaColor(ColorsPrivate::fg_color, variant));
    palette.setColor(QPalette::All,      QPalette::Base,            colorsGlobal->oodwaitaColor(ColorsPrivate::base_color, variant));
    palette.setColor(QPalette::All,      QPalette::AlternateBase,   colorsGlobal->oodwaitaColor(ColorsPrivate::base_color, variant));
    palette.setColor(QPalette::All,      QPalette::ToolTipBase,     colorsGlobal->oodwaitaColor(ColorsPrivate::osd_bg_color, variant));
    palette.setColor(QPalette::All,      QPalette::ToolTipText,     colorsGlobal->oodwaitaColor(ColorsPrivate::osd_text_color, variant));
    ...
    palette.setColor(QPalette::All,      QPalette::Shadow,          colorsGlobal->oodwaitaColor(ColorsPrivate::shadow_color, variant));

    palette.setColor(QPalette::All,      QPalette::Highlight,       colorsGlobal->oodwaitaColor(ColorsPrivate::selected_bg_color, variant));
    palette.setColor(QPalette::All,      QPalette::HighlightedText, colorsGlobal->oodwaitaColor(ColorsPrivate::selected_fg_color, variant));

    palette.setColor(QPalette::All,      QPalette::Link,            colorsGlobal->oodwaitaColor(ColorsPrivate::link_color, variant));
    palette.setColor(QPalette::All,      QPalette::LinkVisited,     colorsGlobal->oodwaitaColor(ColorsPrivate::link_visited_color, variant));

    palette.setColor(QPalette::Disabled, QPalette::Window,          colorsGlobal->oodwaitaColor(ColorsPrivate::insensitive_bg_color, variant));
    palette.setColor(QPalette::Disabled, QPalette::WindowText,      colorsGlobal->oodwaitaColor(ColorsPrivate::insensitive_fg_color, variant));
    palette.setColor(QPalette::Disabled, QPalette::Base,            colorsGlobal->oodwaitaColor(ColorsPrivate::base_color, variant));
    palette.setColor(QPalette::Disabled, QPalette::AlternateBase,   colorsGlobal->oodwaitaColor(ColorsPrivate::base_color, variant));
    ...
    palette.setColor(QPalette::Disabled, QPalette::Shadow,          colorsGlobal->oodwaitaColor(ColorsPrivate::shadow_color, variant));

    palette.setColor(QPalette::Disabled, QPalette::Highlight,       colorsGlobal->oodwaitaColor(ColorsPrivate::selected_bg_color, variant));
    palette.setColor(QPalette::Disabled, QPalette::HighlightedText, colorsGlobal->oodwaitaColor(ColorsPrivate::selected_fg_color, variant));

    palette.setColor(QPalette::Disabled, QPalette::Link,            colorsGlobal->oodwaitaColor(ColorsPrivate::link_color, variant));
    palette.setColor(QPalette::Disabled, QPalette::LinkVisited,     colorsGlobal->oodwaitaColor(ColorsPrivate::link_visited_color, variant));

    palette.setColor(QPalette::Inactive, QPalette::Window,          colorsGlobal->oodwaitaColor(ColorsPrivate::backdrop_bg_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::WindowText,      colorsGlobal->oodwaitaColor(ColorsPrivate::backdrop_fg_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::Base,            colorsGlobal->oodwaitaColor(ColorsPrivate::backdrop_base_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::AlternateBase,   colorsGlobal->oodwaitaColor(ColorsPrivate::backdrop_base_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::ToolTipBase,     colorsGlobal->oodwaitaColor(ColorsPrivate::osd_bg_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::ToolTipText,     colorsGlobal->oodwaitaColor(ColorsPrivate::osd_text_color, variant));
    ...
    palette.setColor(QPalette::Inactive, QPalette::Shadow,          colorsGlobal->oodwaitaColor(ColorsPrivate::shadow_color, variant));

    palette.setColor(QPalette::Inactive, QPalette::Highlight,       colorsGlobal->oodwaitaColor(ColorsPrivate::selected_bg_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::HighlightedText, colorsGlobal->oodwaitaColor(ColorsPrivate::selected_fg_color, variant));

    palette.setColor(QPalette::Inactive, QPalette::Link,            colorsGlobal->oodwaitaColor(ColorsPrivate::link_color, variant));
    palette.setColor(QPalette::Inactive, QPalette::LinkVisited,     colorsGlobal->oodwaitaColor(ColorsPrivate::link_visited_color, variant));
...
{
    return colorsGlobal->oodwaitaColor(ColorsPrivate::focus_border_color, options.colorVariant());
}
...
{
    return colorsGlobal->oodwaitaColor(ColorsPrivate::menu_selected_color, options.colorVariant());
}
...
QColor Colors::buttonOutlineColor(const StyleOptions &options)
{
    ...
    return colorsGlobal->oodwaitaWidgetColor(colorName, options.colorVariant());
}
...
QColor Colors::indicatorOutlineColor(const StyleOptions &options)
{
    ...
    if (options.inMenu()) {
        ...
        if (options.colorVariant() == ColorVariant::OodwaitaDark || options.colorVariant() == OodwaitaHighcontrastInverse) {
...
QColor Colors::buttonBackgroundColor(const StyleOptions &options)
{
    ...
    if (isDisabled && (options.animationMode() == AnimationPressed || options.sunken())) {
        return colorsGlobal->oodwaitaWidgetColor(QStringLiteral("button_disabled_active_background_image"), options.colorVariant());
    }
    ...
    if (options.animationMode() == AnimationPressed) {
        ...
        return Colors::mix(colorsGlobal->oodwaitaWidgetColor(buttonColor, options.colorVariant()),
                           colorsGlobal->oodwaitaWidgetColor(buttonHoverColor, options.colorVariant()), options.opacity());
...
QLinearGradient Colors::buttonBackgroundGradient(const StyleOptions &options)
{
    ...
    QColor gradientStopColor = colorsGlobal->oodwaitaWidgetColor(gradientName, options.colorVariant());
...
QColor Colors::indicatorBackgroundColor(const StyleOptions &options)
{
    ...
    if (options.animationMode() == AnimationPressed) {
        ...
        return Colors::mix(colorsGlobal->oodwaitaWidgetColor(checkradioColor, options.colorVariant()),
                           colorsGlobal->oodwaitaWidgetColor(checkradioHoverColor, options.colorVariant()), options.opacity());
    } else if (options.animationMode() == AnimationHover) {
...
QColor Colors::scrollBarHandleColor(const StyleOptions &options)
{
    ...
    QColor activeColor(options.colorVariant() == ColorVariant::OodwaitaDark || options.colorVariant() == ColorVariant::OodwaitaHighcontrastInverse ? Colors::lighten(selectedBgColor, 0.1) : Colors::darken(selectedBgColor, 0.1));
